...
const ModalEditTicket = ({ showModalEdit, setShowModalEdit, itemTicket, callBack }: IProps) => {
	...
	const {
		...
		defaultValues: {
			fullname: itemTicket?.name,
			username: itemTicket?.username,
			email: itemTicket?.email,
			phone: itemTicket?.phoneNumber,
			role: itemTicket?.role,
		}
	})
	...
	const updatePost = async (data: any) => {
		try {
			const res = await userAPI.updateUser(itemTicket?.id, {
				name: data?.fullname,
	...
	}, [itemTicket, setShowModalEdit, showModalEdit])
	return (
		<>
			<Modal
