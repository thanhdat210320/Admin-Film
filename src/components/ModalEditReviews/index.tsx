...
const ModalEditUser = ({ showModalEdit, setShowModalEdit, itemReviews, callBack }: IProps) => {
	...
	const {
		...
		defaultValues: {
			title: itemReviews?.title,
			genre: itemReviews?.genre,
			trailer: itemReviews?.trailer,
			duration: itemReviews?.duration,
		}
	})
	...
	const updatePost = async (data: any) => {
		try {
			const res = await moviesAPI.updateMovies(itemReviews?.id, {
				title: data?.title,
				genre: data?.genre,
	...
	}, [itemReviews, setShowModalEdit, showModalEdit])
	return (
		<>
			<Modal
