...
const ModalEditScreenings = ({ showModalEdit, setShowModalEdit, itemScreenings, callBack }: IProps) => {
	...
	const {
		...
		defaultValues: {
			title: itemScreenings?.title,
			genre: itemScreenings?.genre,
			trailer: itemScreenings?.trailer,
			duration: itemScreenings?.duration,
		}
	})
	...
	const updatePost = async (data: any) => {
		try {
			const res = await moviesAPI.updateMovies(itemScreenings?.id, {
				title: data?.title,
				genre: data?.genre,
	...
	}, [itemScreenings, setShowModalEdit, showModalEdit])
	return (
		<>
			<Modal
